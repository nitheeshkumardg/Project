  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        {/* Filter Section - Taking Extra Width */}
        ...
        <div className="flex-grow">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredproduct.map((product) => (
              <div
                className="card p-2"  // Removed border-4
                style={{ width: '18rem' }}
                key={product._id}
              >
                <img
                  src={product.image}
                  className="card-img-top w-full h-48 object-cover rounded-lg" // Removed border and border-gray-300 classes
                  alt={product.title}
                />
